 ...
 *
 * Supports English and Arabic voice announcements for:
 * - Turn-by-turn maneuvers (e.g. "In 150 meters, turn right", "Turn right now")
 * - Stop arrivals & transfer warnings
...
export function useVoiceGuidance({
  ...
  const lastSpokenRef = useRef({ text: null, timestamp: 0, distanceBucket: null })
  ...
  useEffect(() => {
    ...
    // Categorize distance buckets for spoken announcements:
    // Bucket 1: approaching ~ 150m-200m
    // Bucket 2: immediate action < 25m
    let bucket = null
    if (dist <= 25) {
      bucket = 'now'
