...
export function SocialAuthButtons({ redirectFrom }) {
  const { t } = useI18n()
  ...
  const handleOAuth = (provider) => {
    ...
    if (redirectFrom) {
      sessionStorage.setItem('wasel.auth_redirect', redirectFrom)
    }
    // Redirect through the API endpoint
    window.location.href = `/api/v1/auth/${provider}/redirect`
  }

  return (
    <div className="social-auth">
      <div className="auth-divider" role="separator" aria-label={t('auth.or')}>
        <span>{t('auth.or')}</span>
      </div>
      ...
      <div className="social-auth__buttons">
        ...
        >
          {loadingProvider === 'github' ? (
            ...
            <>
              <GitHubIcon size={19} />
              <span>{t('auth.continue_github')}</span>
            </>
          )}
        </button>
      </div>
